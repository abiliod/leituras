...
class InspecaoController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $dados = $request->all();
        $registro = ItemDeInspecao::find($id);
        if($dados['situacao'] =="cancel")
        {
           return redirect()-> route('compliance.inspecao',$registro->verificacao_id);
        }
        ...
        return redirect()-> route('compliance.inspecao',$inspecao->verificacao_id);
    }
    ...
    public function editsro(Request $request, $id)
    {
         ...
         $inspecao  =  DB::table('verificacoes')
        ...
        if ($aval < $mostra)
        {
            ...
            $registro = DB::table('itensDeInspecao')
                ->join('verificacoes', 'itensDeInspecao.verificacao_id', '=', 'verificacoes.id')
                ->join('unidades', 'itensDeInspecao.unidade_id', '=', 'unidades.id')
                ...
                ->select('itensDeInspecao.*','verificacoes.*','unidades.*','testesDeVerificacao.*','gruposDeVerificacao.*')
                ->where([['itensDeInspecao.id', '=', $id ]])
                ->first();
...
//dd();
    ...
    public function search (Request $request )
    {
        ...
        // dd($dados);
            ...
            ->where([['verificacao_id', '=', $request->all()['id']]])
          //  ->where([['testesdeverificacao.teste', 'LIKE', '%' . $request->all()['search'] .'%' ]])
            ->Where([['itensDeInspecao.grupoVerificacao_id', '=', $request->all()['gruposdeverificacao']]])
           // ->Where([['itensDeInspecao.status', '=', $request->all()['status']]])
        ...
        }else if(($request->all()['gruposdeverificacao']!=NULL) && ($request->all()['situacao']!=NULL) && ($request->all()['search']==NULL))
        {
          ...
          //  ->where([['testesdeverificacao.teste', 'LIKE', '%' . $request->all()['search'] .'%' ]])
            ->Where([['itensDeInspecao.grupoVerificacao_id', '=', $request->all()['gruposdeverificacao']]])
            ->Where([['itensDeInspecao.situacao', '=', $request->all()['situacao']]])
            ->orderBy('itensDeInspecao.testeVerificacao_id' , 'asc')
           ->paginate(10);
        ...
        }else if(($request->all()['gruposdeverificacao']==NULL) && ($request->all()['situacao']==NULL) && ($request->all()['search']!=NULL))
        {
          ...
          ->orderBy('itensDeInspecao.testeVerificacao_id' , 'asc')
            ...
            ->Where([['itensDeInspecao.situacao', '=', $request->all()['situacao']]])
            ->orderBy('itensDeInspecao.testeVerificacao_id' , 'asc')
            //->orderBy('gruposdeverificacao.numeroGrupoVerificacao', 'asc','itensDeInspecao.id' , 'asc')
           ->paginate(10);
        ...
        $inspecao = Verificacao::find($request->all()['id']);
        ...
        $dado = DB::table('itensDeInspecao')
            ->join('verificacoes', 'itensDeInspecao.verificacao_id', '=', 'verificacoes.id')
            ->join('unidades', 'itensDeInspecao.unidade_id', '=', 'unidades.id')
            ->join('testesDeVerificacao', 'itensDeInspecao.testeVerificacao_id', '=', 'testesDeVerificacao.id')
            ->join('gruposDeVerificacao', 'itensDeInspecao.grupoVerificacao_id', '=', 'gruposDeVerificacao.id')
            ->select('itensDeInspecao.*','verificacoes.*','unidades.*','testesDeVerificacao.*','gruposDeVerificacao.*')
            ->where([['verificacoes.id', '=', $request->all()['id'] ]])
            ->first();
    ...
    public function index($id)  {
        ...
        if($count == 0){
            ...
            $verificacoes = Verificacao::find( $id );
            $verificacoes->status = 'Inspecionado';
            //dd('aki '.  $count);
            $verificacoes->save();
